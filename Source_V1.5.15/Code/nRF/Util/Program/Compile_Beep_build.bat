...
ECHO remove bootloader build and hex files
IF EXIST "../../BTLDR/ses/Output" RMDIR /S /Q "../../BTLDR/ses/Output"

ECHO remove application build and hex files
IF EXIST RMDIR "../../PRJ/ses/Output/" /S /Q "../../PRJ/ses/Output/"


echo Clean-up complete, compiling release version: %build% in folder %compileDir%
...
start /B /wait ../mergehex.exe --merge %compileDir%/BL_settings.hex ../../BTLDR/ses/Output/Release/Exe/beep_bootloader.hex --output %compileDir%/BTLDR_settings.hex
if %ERRORLEVEL% NEQ 0 (
...
start /B /wait ../mergehex.exe --merge ../../BTLDR/ses/Output/Release/Exe/beep_bootloader.hex %compileDir%/APP_SD.hex --output %compileDir%/APP_SD_BL.hex
if %ERRORLEVEL% NEQ 0 (
